...
class WizReport1099(models.TransientModel):
    ...
    start_date = fields.Date(default=lambda self: date(date.today().year, 1, 1))
    end_date = fields.Date(default=lambda self: date(date.today().year, 12, 31))
        ...
        if self.start_date and self.end_date:
            ...
            if self.report_option == "eligible_vendors":
                st_dt = datetime.strptime(
                    str(self.start_date), DEFAULT_SERVER_DATE_FORMAT
                )
                en_dt = datetime.strptime(
                    str(self.end_date), DEFAULT_SERVER_DATE_FORMAT
                )
                date_dict = self.env.user.company_id.compute_fiscalyear_dates(st_dt)
                fiscal_st_dt = date_dict.get("date_from", False)
                fiscal_en_dt = date_dict.get("date_to", False)
                if not (st_dt <= fiscal_en_dt and st_dt >= fiscal_st_dt) or not (
                    en_dt <= fiscal_en_dt and en_dt >= fiscal_st_dt
                ):
                    raise ValidationError(
    ...
    def print_pdf_report(self):
        ...
        return self.env.ref("report_1099.report_1099_vendor").report_action(
            report_1099_recs
        )
    ...
    def get_invoices(self):
        ...
        if self.report_option == "eligible_vendors":
            ...
            for payment_rec in payments_recs:
                ...
                if payment_rec.partner_id.id in pay_eligble_dict:
                    ...
                    pay_eligble_dict.update(
                        {payment_rec.partner_id.id: existed_amt + amt}
                    )
                else:
            ...
            final_eligible_dict = {
                partner: amount
                for partner, amount in pay_eligble_dict.items()
                if amount >= 600
            }
            pay_partners = list(final_eligible_dict.keys())
            eligible_dict = {}
            for pay_rec in payments_recs.filtered(
                lambda p: p.partner_id.id in pay_partners
            ):
                line_total = 0.0
                eligible_invs = []
                ...
                for payment_inv in pay_rec.invoice_ids:
                    inv_ln = payment_inv.invoice_line_ids.filtered(lambda r: r.is_1099)
                ...
                if part_id in eligible_dict:
                    ...
                    eligible_dict.update(
                        {part_id: {"total": line_total, "invs": eligible_invs}}
                    )
        ...
        for inv in invoice_ids:
            ...
            payment_dict_to_show = sorted(
                payment_vals, key=lambda k: k["date"], reverse=True
            )
            payment_dict = {}
            if payment_dict_to_show:
                payment_rec = payment_obj.browse(
                    payment_dict_to_show[0].get("account_payment_id", False)
                )
                payment_dict.update(
                    {
                        ...
                        "invoice_state": inv.invoice_payment_state == "paid"
                        and "paid"
                        or "open",
                    }
                )
                for inv_line in inv.invoice_line_ids.filtered(lambda r: r.is_1099):
                    ...
                    if inv.state != "posted":
                        ...
                        debit_line = payment_rec.move_line_ids.filtered(
                            lambda line: line.account_id.user_type_id.id
                            == payable_acc.id
                        )
                        debit_part_id = debit_line.partner_id.id
                        rep_1099_conf_rec = rep_1099_config_obj.search(
                            [("partner_id", "=", debit_part_id)], limit=1
                        )
                        debit_line_dict = {
                            ...
                            "currency_id": debit_line.currency_id
                            and debit_line.currency_id.id
                            or False,
                        }
        ...
        rep_1099_rec = [rep_1099_obj.create(val).id for val in rep_1099_data_list]
    ...
    def get_report_data(self, docids=False):
        ...
        cr = self._cr
        cr.execute(
            """select partner_id,type_income,sum(amount) from
    ...
    def print_xls_report(self):
        ...
        for partner_id, income_dict in data_dict.items():
            ...
            worksheet = workbook.add_sheet(
                "Report 1099 " + str(partner_name), cell_overwrite_ok=True
            )
            header_bold = xlwt.easyxf("font: bold on; alignment: horiz center;")
            ...
            style_center = xlwt.easyxf("alignment: horiz center;align:vert top;")
            ...
            worksheet.write_merge(
                1, 8, 11, 14, "Miscellaneous\nIncome", style_vert_centre
            )
            ...
            worksheet.write_merge(
                ...
                + "\n\n\n"
                + "$"
                + str(income_dict.get("federal_income_tax_withheld", "")),
                style_wrap,
            )
            worksheet.write_merge(
                9,
            ...
            worksheet.write_merge(
                13, 16, 0, 2, "PAYER’S federal identification number ", style_wrap
            )
            ...
            worksheet.write_merge(
                ...
                + "\n\n\n"
                + "$"
                + str(income_dict.get("fishing_boat_proceeds", "")),
                style_wrap,
            )
            worksheet.write_merge(
                13,
                ...
                + "\n\n\n"
                + "$"
                + str(income_dict.get("medical_and_health_care_payments", "")),
                style_wrap_2,
            )
            ...
            worksheet.write_merge(
                ...
                + "\n\n\n"
                + "$"
                + str(income_dict.get("non_emp_cmpr", "")),
                style_wrap,
            )
            worksheet.write_merge(
                17,
            ...
            street_address = (
                str(partner.street and partner.street or "")
                + " "
                + str(partner.street2 and partner.street2 or "")
            )
            worksheet.write_merge(
                21,
                24,
            ...
            worksheet.write_merge(
                ...
                + "\n\n\n"
                + "$"
                + str(income_dict.get("crop_insurance_proceeds", "")),
                style_wrap,
            )
            ...
            worksheet.write_merge(
                29, 32, 0, 3, "Account number (see instructions)", style_wrap
            )
            worksheet.write_merge(29, 32, 4, 4, "FATCA filing requirement", style_wrap)
            ...
            worksheet.write_merge(
                ...
                + "\n\n"
                + "$"
                + str(income_dict.get("excess_golden_perachute_payments", "")),
                style_wrap,
            )
            worksheet.write_merge(
                29,
                ...
                + "\n\n\n"
                + "$"
                + str(income_dict.get("gross_proceeds_paid_to_an_attomey", "")),
                style_wrap,
            )
            ...
            worksheet.write_merge(
                ...
                + "\n\n\n"
                + "$"
                + str(income_dict.get("section_409A_deferrals", "")),
                style_wrap,
            )
            worksheet.write_merge(
                33,
                ...
                + "\n\n\n"
                + "$"
                + str(income_dict.get("section_409A_income", "")),
                style_wrap,
            )
            worksheet.write_merge(
                33,
                ...
                + "\n\n\n"
                + "$"
                + str(income_dict.get("state_tax_withheld", "")),
                style_wrap,
            )
            worksheet.write_merge(
                33,
                ...
                + "\n\n\n"
                + "$"
                + str(income_dict.get("state_payers_state_no", "")),
                style_wrap,
            )
            worksheet.write_merge(
                33,
                ...
                + "\n\n\n"
                + "$"
                + str(income_dict.get("state_income", "")),
                style_wrap,
            )
